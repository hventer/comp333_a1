...
class VariableCollection extends TreeMap<String, Variable> {
	...
	public TreeMap<String, Integer> registerAllocation() {
		TreeMap<String, Integer> variableRegister = new TreeMap<String, Integer>();
		...
		for(Variable variable : this.values())  {
			...
			for(int i=0; i<variable.getNeighbourNames().size(); i++) {
				//all the neighbours of this register

				if(getVariableByName(variable.getNeighbourNames().get(i)).getRegister() == register) {
					//this neighbour variable already uses this register
					
					register++; //increase the register
					i=-1; //re-check neighbour's registers for new register
				}
			}
			variable.setRegister(register);
			variableRegister.put(variable.getName(), register);
		}
